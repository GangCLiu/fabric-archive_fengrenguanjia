...
def get_ocr():
    ...
    # 避免 Streamlit 反复 rerun 时重复触发高成本初始化，形成“看起来像死循环”的重试风暴
    if _ocr_init_attempted and _ocr_init_error is not None:
    ...
    try:
        ...
        _ocr = None
        _ocr_init_error = (
            f"OCR初始化失败（本次进程已停止重复重试，避免循环卡顿）: {e}. "
            "请重启应用后重试；如仍失败，请手动删除 ~/.paddlex/official_models 后再启动。"
        )
        print(f"❌ {_ocr_init_error}")
        raise RuntimeError(_ocr_init_error)
